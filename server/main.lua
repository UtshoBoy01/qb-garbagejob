...
QBCore.Functions.CreateCallback("garbagejob:server:NextStop", function(source, cb, currentStop, currentStopNum, currLocation)
    ...
    if(math.random(25) >= Config.CryptoStickChance) and Config.GiveCryptoStick then
    ...
    elseif(math.random(35) >= Config.CryptoStickChance) and Config.GiveCryptoStick then
        Player.Functions.AddItem("metalscrap", 1, false)
        TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items["metalscrap"], 'add')
    elseif(math.random(45) >= Config.CryptoStickChance) and Config.GiveCryptoStick then
        Player.Functions.AddItem("rubber", 1, false)
        TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items["rubber"], 'add')
    elseif(math.random(60) >= Config.CryptoStickChance) and Config.GiveCryptoStick then
        Player.Functions.AddItem("rubber", 1, false)
        TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items["rubber"], 'add')
    elseif(math.random(85) >= Config.CryptoStickChance) and Config.GiveCryptoStick then
        Player.Functions.AddItem("sandwich", 1, false)
        TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items["laptop"], 'add')
    elseif(math.random(90) >= Config.CryptoStickChance) and Config.GiveCryptoStick then
        Player.Functions.AddItem("joint", 1, false)
        TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items["hacking_device"], 'add')
    else
        TriggerClientEvent('QBCore:Notify', source, 'You Found Nothing!', "error")
    end
